...
static struct RSP {
    ...
    uintptr_t segment_addrs[16];
    
    union Vec4b vertex_color_table[64];
} rsp;
...
static void gfx_sp_vertex(size_t n_vertices, size_t dest_index, const Vtx *vertices) {
    for (size_t i = 0; i < n_vertices; i++, dest_index++) {
        ...
        union Vec4b* normcolor = &rsp.vertex_color_table[v->colour];
        ...
        if (rsp.geometry_mode & G_LIGHTING) {
            ...
            for (int i = 0; i < rsp.current_num_lights - 1; i++) {
                float intensity = 0;
                intensity += normcolor->x * rsp.current_lights_coeffs[i][0];
                intensity += normcolor->y * rsp.current_lights_coeffs[i][1];
                intensity += normcolor->z * rsp.current_lights_coeffs[i][2];
                intensity /= 127.0f;
                if (intensity > 0.0f) {
            ...
            if (rsp.geometry_mode & G_TEXTURE_GEN) {
                float dotx = 0, doty = 0;
                dotx += normcolor->x * rsp.current_lookat_coeffs[0][0];
                dotx += normcolor->y * rsp.current_lookat_coeffs[0][1];
                dotx += normcolor->z * rsp.current_lookat_coeffs[0][2];
                doty += normcolor->x * rsp.current_lookat_coeffs[1][0];
            ...
            d->color.r = normcolor->r;
            d->color.g = normcolor->g;
            d->color.b = normcolor->b;
        }
...
static inline void *seg_addr(const uintptr_t w1) {
    ...
    if (seg > 0 && rsp.segment_addrs[seg] > 0) {
        ...
        if (addr > (512 * 1024))
            return (void *) w1;
        return (void *)(addr + rsp.segment_addrs[seg]);
    }
    return (void *) w1;
}
    ...
    for (;;) {
        ...
        switch (opcode) {
...
#else
                gfx_sp_vertex(C0(12, 4), C0(8, 4), seg_addr(cmd->words.w1));
#endif
                break;
            case G_DL:
                if (C0(16, 1) == 0) {
            ...
            case (uint8_t)G_SETOTHERMODE_H:
                ...
                break;
            case G_COL:
                gfx_sp_set_vertex_colors(C0(0, 16) / 4, seg_addr(cmd->words.w1));
                break;
            ...
            case G_RDPTILESYNC:
                break;
            default:
                fprintf(stderr, "Unhandled opcode: 0x%08x\n", opcode);
                break;
        }
        ++cmd;
    }
...
static void gfx_sp_reset() {
    ...
    for (size_t i = 0; i < sizeof(rsp.segment_addrs) / sizeof(*rsp.segment_addrs); i++)
