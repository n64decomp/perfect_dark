...
static struct RDP {
    ...
    std::unordered_map<uint16_t, LoadedTexture> loaded_texture;
    struct {
        uint8_t fmt;
        uint8_t siz;
        uint8_t cms, cmt;
...
static void import_texture_rgba16(int tile, const LoadedTexture& loaded_texture) {
    ...
    const uint32_t full_image_line_size_bytes = loaded_texture.full_image_line_size_bytes;
...
static void import_texture(int i, int tile, bool importReplacement) {
    ...
    const uint32_t tmem_index = rdp.texture_tile[tile].tmem;
    const uint8_t palette_index = rdp.texture_tile[tile].palette;
    LoadedTexture& loaded_texture = rdp.loaded_texture[tmem_index];
    ...
    if (fmt == G_IM_FMT_RGBA) {
        if (siz == G_IM_SIZ_16b) {
            import_texture_rgba16(tile, loaded_texture);
        } else if (siz == G_IM_SIZ_32b) {
            import_texture_rgba32(tile, loaded_texture);
        } else {
            abort();
        }
    } else if (fmt == G_IM_FMT_IA) {
        if (siz == G_IM_SIZ_4b) {
            import_texture_ia4(tile, loaded_texture);
        } else if (siz == G_IM_SIZ_8b) {
            import_texture_ia8(tile, loaded_texture);
        } else if (siz == G_IM_SIZ_16b) {
            import_texture_ia16(tile, loaded_texture);
        } else {
            abort();
        }
    } else if (fmt == G_IM_FMT_CI) {
        if (siz == G_IM_SIZ_4b) {
            import_texture_ci4(tile, loaded_texture);
        } else if (siz == G_IM_SIZ_8b) {
            import_texture_ci8(tile, loaded_texture);
        } else {
            abort();
        }
    } else if (fmt == G_IM_FMT_I) {
        if (siz == G_IM_SIZ_4b) {
            import_texture_i4(tile, loaded_texture);
        } else if (siz == G_IM_SIZ_8b) {
            import_texture_i8(tile, loaded_texture);
        } else {
            abort();
        }
    } else {
...
static void gfx_dp_load_tile(uint8_t tile, uint32_t uls, uint32_t ult, uint32_t lrs, uint32_t lrt) {
    ...
    if (rdp.tex_lod) {
        ...
        rdp.textures_changed[rdp.texture_tile[tile].tmem != 0] = true;
    }
}
