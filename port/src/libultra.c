...
s32 __osMotorAccess(OSPfs *pfs, s32 cmd)
{
	...
	const f32 strength = (cmd == MOTOR_START) ? INPUT_RUMBLE_STRENGTH : 0.f;
...
#ifndef HAVE_BCOPY
void bcopy(const void *src, void *dst, s32 n)
{
	memcpy(dst, src, n);
}
#endif
...
#ifndef HAVE_BCMP
s32 bcmp(const void *s1, const void *s2, s32 n)
{
